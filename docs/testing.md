...
| `tests/test_project_instructions.cpp` | `agent-instructions/PROJECT_INSTRUCTIONS.md` — stage authority, read-only assessment stages, the candidate publication gate, checkout-free report publication and its workspace-root fallback ban, single-owner exact-head evidence, the headless background/retry prohibitions, and the CTest node-age route; each pin proved sensitive by re-checking a copy with that clause removed |
