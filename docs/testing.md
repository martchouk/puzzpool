...
| `tests/test_project_instructions.cpp` | `agent-instructions/PROJECT_INSTRUCTIONS.md` — stage authority, read-only assessment stages, the candidate publication gate and each of its prerequisites, checkout-free report publication with every forbidden answer to a refusal, the workspace-root fallback ban, single-owner exact-head evidence, the headless background/detachment/retry prohibitions and the foreground durable-capture route, and the CTest node-age route; each pin proved sensitive by re-checking a copy with that clause removed, and the four multi-part obligations additionally replayed as deletions that leave the surrounding sentence readable, singly and all at once |
